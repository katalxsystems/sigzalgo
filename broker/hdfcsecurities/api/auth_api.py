# broker/hdfcsecurities/api/auth_api.py
#
# HDFC Securities InvestRight login (browser-redirect flow):
...
from broker.hdfcsecurities.api.baseurl import get_hdfcsecurities_headers, get_root_url
...
def authenticate_broker(request_token, account_id=None):
    """Exchange the InvestRight request token for an access token.

    Returns:
        (auth_token, error_message) -- exactly one of the two is set.
    """
    try:
        api_key = get_broker_api_key(account_id, broker="hdfcsecurities")
        api_secret = get_broker_api_secret(account_id, broker="hdfcsecurities")

        if not api_key or not api_secret:
            return None, "BROKER_API_KEY / BROKER_API_SECRET are not configured"
        if not request_token:
            return None, "No request token received from HDFC Securities callback"

        client = get_httpx_client()
        url = f"{get_root_url()}/oapi/v1/access-token"
        # `Authorization` is not available yet at this point; drop it so the
        # mandatory User-Agent / Content-Type still go out.
        headers = get_hdfcsecurities_headers("", with_json=True)
        ...
        try:
            ...
            return None, (
                f"HDFC Securities returned a non-JSON response "
        ...
        return None, f"HDFC Securities authentication failed: {message}"

    except Exception as e:
        logger.exception(f"HDFC Securities authentication error: {e}")
